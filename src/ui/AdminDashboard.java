...
public class AdminDashboard extends JFrame {
    ...
    private JTable tblCustomer, tblAccount;
    ...
    private JPanel createCustomerPanel() {
        ...
        btnPanel.add(btnAdd); btnPanel.add(btnEdit); btnPanel.add(btnDel); btnPanel.add(btnBack);
        ...
        btnAdd.addActionListener(e -> {
            ...
            if (name.isEmpty() || phone.isEmpty() || cardId.isEmpty()) {
                JOptionPane.showMessageDialog(this, "Vui lòng nhập Tên, SĐT và CCCD!", "Thiếu thông tin", JOptionPane.WARNING_MESSAGE);
                return;
            }

            Customer tmpCust = new Customer(0, name, dob, address, phone, cardId);
            String errorMsg = CustomerService.validateCustomer(tmpCust);

            if (errorMsg != null) {
                JOptionPane.showMessageDialog(this, errorMsg, "Trùng dữ liệu", JOptionPane.ERROR_MESSAGE);
                return;
            }
            // ----------------------------------------
            if (CustomerService.addCustomer(new Customer(0, name, dob, address, phone, cardId))) {
                JOptionPane.showMessageDialog(this, "Thêm thành công!");
                loadCustomerData("");
                clearForm();
            } else {
                JOptionPane.showMessageDialog(this, "Lỗi thêm! (Có thể do lỗi kết nối Database)");
            }
        });

        btnEdit.addActionListener(e -> {
            if (selectedCustomerId == -1) {
                JOptionPane.showMessageDialog(this, "Vui lòng chọn khách hàng cần sửa!", "Chưa chọn dòng", JOptionPane.WARNING_MESSAGE);
                return;
            }

            // 2. Lấy dữ liệu và loại bỏ khoảng trắng
            String name = txtName.getText().trim();
            String phone = txtPhone.getText().trim();
            String cardId = txtCardId.getText().trim();
            String dob = txtDob.getText().trim();
            ...
            // 3. Kiểm tra thông tin rỗng
            if (name.isEmpty() || phone.isEmpty() || cardId.isEmpty()) {
                JOptionPane.showMessageDialog(this, "Tên, SĐT và CCCD không được để trống!", "Thiếu thông tin", JOptionPane.WARNING_MESSAGE);
                return;
            }

            // 4. Thực hiện cập nhật
            Customer upd = new Customer(selectedCustomerId, name, dob, address, phone, cardId);
            if (CustomerService.updateCustomer(upd)) {
                JOptionPane.showMessageDialog(this, "Sửa thành công!");
                loadCustomerData("");
                clearForm();
                selectedCustomerId = -1;
            } else {
                JOptionPane.showMessageDialog(this, "Lỗi cập nhật (Có thể trùng SĐT/CCCD)!", "Lỗi", JOptionPane.ERROR_MESSAGE);
            }
        });

        btnDel.addActionListener(e -> {
            if (selectedCustomerId == -1) {
                JOptionPane.showMessageDialog(this, "Vui lòng chọn khách hàng cần xóa!", "Chưa chọn dòng", JOptionPane.WARNING_MESSAGE);
                return;
            }
            int confirm = JOptionPane.showConfirmDialog(this,
                    "Bạn có chắc chắn muốn xóa khách hàng này?\nLƯU Ý: Toàn bộ tài khoản và lịch sử giao dịch của họ cũng sẽ bị xóa!",
            ...
            if (confirm == JOptionPane.YES_OPTION) {
                if (CustomerService.deleteCustomer(selectedCustomerId)) {
                    ...
                    loadCustomerData("");
                    clearForm();
                    selectedCustomerId = -1; // Reset biến ID
                } else {
    ...
    private JTextField txtEditAccNumber, txtEditAccBalance;
    ...
    private JPanel createAccountPanel() {
        ...
        btnCreate.addActionListener(e -> {
            try {
                ...
                if (AccountService.isAccountExist(accId)) { JOptionPane.showMessageDialog(this, "Trùng số TK!"); return; }
                if (!AccountService.isCustomerExist(Integer.parseInt(custIdStr))) { JOptionPane.showMessageDialog(this, "Khách hàng không tồn tại!"); return; }

                // Gọi hàm service tạo tài khoản
                if (AccountService.createAccount(new Account(null, accId, "", bal), Integer.parseInt(custIdStr))) {
        ...
        btnUpdate.addActionListener(e -> {
            ...
            try {
                // 2. Gọi hàm vừa viết
                if (AccountService.updateAccount(new Account(null, accId, "", newBalance), isLocked)) {
        ...
        btnDeleteAcc.addActionListener(e -> {
            if(!txtEditAccNumber.getText().isEmpty() && JOptionPane.showConfirmDialog(this,"Xóa?")==0) {
                AccountService.deleteAccount(txtEditAccNumber.getText()); loadAccountData(null);
            }
        });
    ...
    private void loadCustomerData(String keyword) {
        ...
        try (ResultSet rs = CustomerService.searchCustomers(keyword)) {
    ...
    private void loadAccountData(Integer customerId) {
        ...
        ResultSet rs = null;
        try {
            if (customerId == null) rs = AccountService.getAllAccounts();
            else rs = AccountService.getAccountsByCustomerId(customerId);
    ...
    private void loadTransactionData() {
        ...
        try (ResultSet rs = TransactionService.getAllTransactions()) {
            while (rs != null && rs.next()) {
                Vector<Object> v = new Vector<>();
    ...
    private void loadCustomersToCombo(JComboBox<ComboItem> cbo) {
        cbo.removeAllItems();
        try (ResultSet rs = CustomerService.getAllCustomers()) {
            try(ResultSet rs2 = CustomerService.searchCustomers("")) {
    ...
    private void setupComboEvents(JComboBox<ComboItem> cboCust, JComboBox<ComboItem> cboAcc, JLabel lblBal) {
        cboCust.addActionListener(e -> {
            ...
            if (item != null) {
                ...
                try (ResultSet rs = AccountService.getAccountsByCustomerId(Integer.parseInt(item.getKey()))) {
                    while (rs != null && rs.next()) {
    ...
    private void processTransaction(String type, JComboBox<ComboItem> cboSource, JTextField txtAmt, JComboBox<ComboItem> cboTarget) {
        try {
            ...
            if (TransactionService.isAccountLocked(srcId)) {
            ...
            if ("NAP".equals(type)) {
                success = TransactionService.deposit(srcId, amount);
                ...
                success = TransactionService.withdraw(srcId, amount);
                ...
                if (TransactionService.isAccountLocked(targetId)) {
                    JOptionPane.showMessageDialog(this,
                ...
                String result = TransactionService.transferWithReason(srcId, targetId, amount);
                if ("SUCCESS".equals(result)) {
                    ...
                    switch (result) {
                        ...
                        case "ACCOUNT_LOCKED":
                            JOptionPane.showMessageDialog(this,
                                    "Giao dịch thất bại!\nMột trong các tài khoản đang bị KHÓA.",
                                    "Lỗi", JOptionPane.ERROR_MESSAGE);
                            return;
                        case "INSUFFICIENT_FUNDS":
                            ...
                            return;
                        case "DB_ERROR":
                            JOptionPane.showMessageDialog(this, "Giao dịch thất bại do lỗi hệ thống (DB).");
                            return;
                        default:
                            JOptionPane.showMessageDialog(this, "Giao dịch thất bại! Lỗi không xác định.");
                            return;
                    }
                }
            }
    ...
    private void loadStatementData(String accId) {
        ...
        try {
            if (accId.isEmpty()) {
                rs = TransactionService.getAllTransactions();
            } else {
                rs = TransactionService.getTransactionsByAccount(accId);
            }
            ...
            while (rs != null && rs.next()) {
                ...
                v.add(displayType);
                v.add(String.format("%,.0f đ", rs.getDouble("amount")));
                String related = rs.getString("related_account_id");
                v.add(related != null ? related : "-");
